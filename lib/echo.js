...
var defaultNestFactory = new NestFactory();
var nestFactory = defaultNestFactory;

function NestFactory() {}

NestFactory.prototype.make = function() {
  return new echonest.Echonest(params);
}
...
var trackCounters = {};
function getOpsInLastMinute() {
  if (nestFactory !== defaultNestFactory) {
    return 0;
  }
  
  var toRemove = [], sum = 0, now = Date.now();
  Object.keys(trackCounters).sort().forEach(function(stamp) {
    if (now - stamp > 60000) {
      toRemove.push(stamp);
    } else {
      sum += trackCounters[stamp];
    }
  });
  toRemove.forEach(function(stamp) {
    delete trackCounters[stamp];
  });
  return sum;
}

exports.getSimilarArtists = function(artistName, callback) {
  async.whilst(function() {
    return getOpsInLastMinute() > 40;
  }, function(callback) {
    setTimeout(function() {
      callback(null);
    }, 2000);
    ...
    if (err) {
      ...
      var now = Date.now();
      if (!trackCounters[now]) {
        trackCounters[now] = 1;
      } else {
        trackCounters[now] = trackCounters[now] + 1;
      }
      doGetSimilarArtists(artistName, callback);
    }
  });
}

var stripRegex = new RegExp('[^a-zA-Z0-9]', 'gm');
function doGetSimilarArtists(artistName, callback) {
  var nest = nestFactory.make();
  nest.artist.similar({
    format: 'json',
    name: artistName,
    ...
    //dumpJsonSync('tests/data/echonest_artist_similar_to_' + artistName + '.json', JSON.stringify(results, null, 4));
    //dumpJsonSync('/tmp/dump/echonest_artist_similar_to_' + artistName.replace(stripRegex, '_') + '.json', JSON.stringify(results, null, 4));
    if (err) {
      console.log('ERROR WHILE GETTING SIMILAR');
      console.log(Object.keys(err));
      console.log(err);
      callback(err, null);
    } else if (results.status && results.status.code === 0) {
      // aritsts have name, id.
      callback(null, results.artists.map(function(artist) { return artist.name; }));
    } else if (results.status && results.status.code === 5) {
      // unknown artist. report no similars.
      callback(null, []);
    } else {
      console.log(results);
      callback(new Error('Unexpected answer'), null);
    }
  });
}

function dumpJsonSync(path, json) {
  if (json) {
    fs.writeFileSync(path, json, 'utf8');
  }
}

exports.setNestFactoryUnsafe = function(factory) {
  if (factory) {
    nestFactory = factory;
  } else {
    nestFactory = defaultNestFactory;
  }
}
