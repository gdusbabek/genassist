...
// todo: some how make use of the rate limiting headers.
...
exports.getSimilarArtists = function(artistName, callback) {
  ...
  nest.artist.similar({
    ...
    if (err) {
      ...
      console.log(results._httpMeta);
      callback(err, null);
    } else if (results.status && results.status.code === 0) {
      // aritsts have name, id.
      artists = results.artists.map(function(artist) { return artist.name; });
      // this is an unspeakable hack.
      artists._meta = {};
      artists._meta.limitRemaining = parseInt(results._httpMeta.headers['X-Ratelimit-Remaining'], 10);
      artists._meta.limitTotal = parseInt(results._httpMeta.headers['X-Ratelimit-Limit'], 10);
      artists._meta.limitUsed = parseInt(results._httpMeta.headers['X-Ratelimit-Used'], 10);
