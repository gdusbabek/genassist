...
Database.migrate = function(options, callback) {
  ...
  async.waterfall([
    ...
    function migrateLoop(oldVersion, callback) {
      ...
      results.requestedVersion = options.currentVersion;
      async.whilst(function() {
        return curVersion < options.currentVersion;
      }, function(callback) {
        curVersion += 1;
        try {
          ...
          console.log('busted at ' + curVersion + ',' + options.currentVersion);
          callback(ex);
        }
      }, callback);
    },
...
Database.getShared = function(dbExports, callback) {
  Database.initDb(dbExports.options, function(err) {
    if (err) { callback(err); return; }
    callback(null, new dbExports.Constructor(dbExports.options.db));
  })
}
