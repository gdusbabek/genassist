...
Client.prototype.getTopArtists = function(user, callback) {
  var self = this;
  self.execute('user.getTopArtists', {
    user: user,
    period: '12month',
    limit: 25
  }, function(err, response) {
    if (err) {
      ...
      callback(new Error(response.message));
    } else if (response.hasOwnProperty('topartists')) {
      // go to town
      callback(null, response.topartists.artist);
    } else{
      callback(new Error('Unexpected: ' + JSON.stringify(response)));
    }
  });
