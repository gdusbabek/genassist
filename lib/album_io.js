var fs = require('fs');

var async = require('async');
var Rdio = require('rdio-node').Rdio;

var settings = require('./config').settings;
var echo = require('./echo');

var rdio = new Rdio({
  consumerKey: settings.RDIO_KEY,
  consumerSecret: settings.RDIO_SECRET
});
var __rdio = rdio;

var THIRTY_DAYS = 1000 * 60 * 60 * 24 * 30;

function dumpJsonSync(path, json) {
  fs.writeFileSync(path, json, 'utf8');
}

// expects (err, [albumObj])
exports.collectNewAlbums = function(callback) {
  var start = 1, // starting at one is apparently quite important.
      albums = [];
  async.whilst(function() {
    return start !== null;
  }, function(callback) {
    rdio.makeRequest('getNewReleases', {
      time: 'thisweek',
      start: start,
      count: 200
    }, function(err, res) {
      //dumpJsonSync('/tmp/dump/collect_new_albums.json', JSON.stringify(res, null, 4));
      if (err) {
        callback(err);
      } else if (res.status === 'ok') {
        // res.result is the array of albums.
        if (res.result.length == 0) {
          // this causes the whilst to stop.
          start = null;
        } else {
          start += res.result.length;
          res.result.forEach(function(album) { albums.push(album); });
        }
        callback(null);
      } else {
        console.log('Unexpected result while pulling new albums');
        console.log(arguments);
        callback(new Error('Unexpected result while pulling new albums'));
      }
    });
  }, function(err) {
    if (err) {
      callback(err);
    } else {
      callback(null, albums);
    }
  });
}

exports.saveAlbumsLocally = function(path, albums, callback) {
  fs.writeFile(path, JSON.stringify(albums, null, '  '), 'utf8', callback);  
}

// expects(err, albumObjArr)
exports.loadAlbumsFromPath = function(path, callback) {
  fs.readFile(path, 'utf8', function(err, data) {
    var arr = [];
    if (data) {
      arr = JSON.parse(data);
    }
    callback(err, arr);
  });
}

exports.saveFileToDatabase = function(path, db, callback) {
  async.waterfall([
    fs.readFile.bind(fs, path, 'utf8'),
    function parse(json, callback) {
      callback(null, JSON.parse(json));
    },
    exports.saveAlbumsDatabase.bind(null, db)
  ], callback);
}

// expects(err, count)
exports.saveAlbumsDatabase = function(albums, db, callback) {
  var count = 0;
  async.forEachLimit(albums, 1, function(album, callback) {
    async.waterfall([
      db.albumExists.bind(db, album.key),
      function maybeSave(exists, callback) {
        if (exists) {
          callback(null);
        } else {
          db.saveAlbum(album, function(err) {
            if (!err) {
              count += 1;
            }
            callback(err);
          });
        }
      }
    ], callback);
  }, function(err) {
    callback(err, count);
  });
}

// expects(err, json).
function loadJsonOrNothing(path, callback) {
  async.waterfall([
    function checkExists(callback) {
      fs.exists(path, function(exists) {
        callback(null, exists);
      });
    },
    function readIfExists(exists, callback) {
      if (exists) {
        fs.readFile(path, 'utf8', callback);
      } else {
        callback(null, '[]');
      }
    }
  ], callback);
}
// expects(err, newAlbumObjects);
exports.extractNewAlbums = function(oldPath, newPath, callback) {
  // consider just the keys.
  async.waterfall([
    function loadThem(callback) {
      async.parallel([
        loadJsonOrNothing.bind(null, oldPath),
        loadJsonOrNothing.bind(null, newPath)
      ], callback);
    },
    function load(jsons, callback) {
      var oldKeys = JSON.parse(jsons[0]).map(function(o) {return o.key;}),
          newAlbums = JSON.parse(jsons[1]),
          newKeys = newAlbums.map(function(o) {return o.key;});
      oldKeys.forEach(function(k1) {
        var index = newKeys.indexOf(k1);
        if (index >= 0) {
          newKeys.splice(index, 1);
        }
      });
      callback(null, newAlbums.filter(function(obj) { return newKeys.indexOf(obj.key) >= 0; }));
    }
  ], callback);
}

// expects(err, artistNameArray)
exports.getSimilarArtists = function(artist, db, callback) {
  // check to see if the db is recent (30 days)
  var now = Date.now();
  async.waterfall([
    function checkForCacheInDb(callback) {
      db.getSimilarArtists(artist, function(err, lastPoll, artistNameArr) {
        if (err) { callback(err); return; }
        
        if (now - lastPoll < THIRTY_DAYS || artistNameArr.length === 0) {
          var timeout = true, tries = 0, arr = null;
          async.whilst(function() { return timeout && tries < 3; }, function(callback) {
            echo.getSimilarArtists(artist, function(err, artistNameArr) {
              tries += 1;
              if (err && err.code === 'ETIMEDOUT') {
                timeout = true;
                callback(null);
              } else if (err) {
                callback(err);
              } else {
                timeout = false;
                arr = artistNameArr;
                callback(null);
              }
            });  
          }, function(err) {
            callback(err, arr, true);
          });
        } else {
          callback(err, artistNameArr, false);
        }
      });
    },
    function maybeSaveSimilarAritsts(artistNameArr, needsSave, callback) {
      if (needsSave) {
        // RelatedDb.prototype.setSimilarArtists = function(artist, similars, last_poll, callback) {
        db.setSimilarArtists(artist, artistNameArr, new Date(now), function(err) {
          callback(err, artistNameArr);
        });
      } else {
        callback(null, artistNameArr);
      }
    }
  ], callback);
}

exports.setRdioUnsafe = function(newRdio) {
  if (newRdio) {
    rdio = newRdio;
  } else {
    rdio = __rdio;
  }
}

exports.setNestFactoryUnsafe = function(factory) {
  echo.setNestFactoryUnsafe(factory);
}


// get the database for this environment.


/*
sample result:

{ baseIcon: 'album/4/7/1/000000000026d174/1/square-200.jpg',
  releaseDate: '2013-01-22',
  artistUrl: '/artist/The_Hit_Crew/',
  duration: 5518,
  isClean: false,
  shortUrl: 'http://rd.io/x/Qj5kjjc',
  canStream: true,
  embedUrl: 'https://rd.io/e/Qj5kjjc',
  type: 'a',
  price: '9.99',
  key: 'a2543988',
  icon: 'http://cdn3.rd.io/album/4/7/1/000000000026d174/1/square-200.jpg',
  canSample: true,
  name: 'The Best Pop Hits of 1974, Vol. 2',
  isExplicit: false,
  artist: 'The Hit Crew',
  url: '/artist/The_Hit_Crew/album/The_Best_Pop_Hits_of_1974%2C_Vol._2/',
  artistKey: 'r68',
  length: 24,
  trackKeys: 
   [ 't27114011',
     't27114012',
     't27114013',
     't27114014',
     't27114015',
     't27114016',
     't27114017',
     't27114018',
     't27114019',
     't27114020',
     't27114021',
     't27114022',
     't27114023',
     't27114024',
     't27114025',
     't27114026',
     't27114027',
     't27114028',
     't27114029',
     't27114030',
     't27114031',
     't27114032',
     't27114033',
     't27114034' ],
  canTether: true,
  displayDate: 'Jan 22, 2013' 
}
  
 */
